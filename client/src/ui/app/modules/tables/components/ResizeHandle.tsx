...
import { ColumnModel, ISetColumnWidth } from '../types';
...
// TODO: reimplement using HammerJS
export class ResizeHandle extends React.Component<ResizeHandleProps, ResizeHandleState> {
  ...
  componentDidMount() {
    this.manager = new Hammer.Manager(this.div.current, {
      recognizers: [
        [Hammer.Pan],
        [Hammer.Tap, { taps: 2, event: 'doubletap' }]
      ]
    });
  ...
  onDragMove = (event: MouseEvent) => {
    // this.setState(state => ({ offsetX: event.clientX - state.startX }));

    const newWidth = event.clientX - this.state.startX + this.state.initialWidth;
    this.setState({ startX: event.clientX, initialWidth: newWidth });
  ...
  onDragMove = (event: typeof Hammer.Input) => {
    const pointerOffsetX = (event.srcEvent as PointerEvent).clientX - this.state.startX;
    const newOffsetX = Math.max(RESIZE_MIN_WIDTH - this.state.initialWidth, pointerOffsetX);
    this.setState(state => ({ offsetX: newOffsetX }));
  }
  ...
  onDragEnd = (event: typeof Hammer.Input) => {
    ...
    console.info('COLUMN WIDTH', newWidth);
  ...
  render() {
    return (
      <div
        style={{ flexShrink: 0, height: '20px', width: '10px', cursor: 'col-resize' }}
        ref={this.div}
      >
        {this.state.isDragging ? createPortal(
          <div
            style={{
              position: 'absolute',
              left: this.state.offsetX + this.state.startX - this.state.initialLeft,
              width: '5px',
              top: 6,
              height: 'calc(100% - 6px)',
              zIndex: 2,
